    ...
    <form id="formAvaliar">
        ...
        <div class="avaliar-card">
            ...
            <div class="estrelas-input" id="estrelasGeral">
                @for($i = 1; $i <= 5; $i++)
                <span class="estrela" onclick="selecionarEstrela('geral', {{ $i }})">★</span>
                @endfor
            </div>
        ...
        <div class="avaliar-card">
            ...
            <div class="estrelas-input" id="estrelas{{ ucfirst($campo) }}">
                @for($i = 1; $i <= 5; $i++)
                <span class="estrela" onclick="selecionarEstrela('{{ $campo }}', {{ $i }})">★</span>
                @endfor
            </div>
        ...
        <div class="avaliar-card">
            ...
            <textarea name="comentario" class="campo-input" rows="3"
                placeholder="Conte o que achou do pedido..."></textarea>
        </div>

        <div class="avaliar-card">
            <p class="avaliar-label">O que você gostou?</p>
            <div class="avaliar-tags">
        ...
        <div class="avaliar-acoes">
            <button type="submit" class="btn btn-primario w-100" id="btnEnviar">
...
.estrela.ativa { color: #ffc107; transform: scale(1.1); }
...
function selecionarEstrela(campo, nota) {
    const m = { geral: 'notaGeral', comida: 'notaComida', entrega: 'notaEntrega', embalagem: 'notaEmbalagem' };
    const divId = 'estrelas' + campo.charAt(0).toUpperCase() + campo.slice(1);
    document.getElementById(m[campo]).value = nota;
    document.querySelectorAll(`#${divId} .estrela`).forEach((el, i) => {
        el.classList.toggle('ativa', i < nota);
    });
}

function toggleTag(btn, tag) {
    if (tagsSel.has(tag)) { tagsSel.delete(tag); btn.classList.remove('selecionada'); }
    else { tagsSel.add(tag); btn.classList.add('selecionada'); }
    document.getElementById('tagsInput').value = JSON.stringify([...tagsSel]);
}
...
document.getElementById('formAvaliar').addEventListener('submit', async function(e) {
    ...
    if (!nota) { erro.textContent = 'Selecione pelo menos a nota geral.'; erro.style.display = ''; return; }
    ...
    try {
        ...
        if (data.sucesso) {
            ...
            setTimeout(() => window.location.href = '{{ route("cliente.pedidos.show", $pedido) }}', 1000);
        } else {
            erro.textContent = data.erro || 'Erro ao enviar.';
            erro.style.display = '';
            btn.disabled = false;
